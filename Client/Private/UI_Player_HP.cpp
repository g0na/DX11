...
CUI_Player_HP::CUI_Player_HP(const CUI_Player_HP& Prototype)
	: CUIObj { Prototype }
{
}
...
void CUI_Player_HP::Update_Priority(_float fTimeDelta)
{
}

void CUI_Player_HP::Update(_float fTimeDelta)
{
	m_fRatio = m_pPlayer->Get_HpRatio();
}

void CUI_Player_HP::Update_Late(_float fTimeDelta)
{
	if (m_bVisible == false)
		return;
...
HRESULT CUI_Player_HP::Ready_Components()
{
	// Com_VIBuffer 추가
...
HRESULT CUI_Player_HP::Bind_ShaderResources()
{
...
CGameObject* CUI_Player_HP::Clone(void* pArg)
{
	...
	{
		MSG_BOX("Failed to Clone : CUI_Player_HP");
		Safe_Release(pInstance);
	}
