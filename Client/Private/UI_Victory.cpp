...
CUI_Victory::CUI_Victory(const CUI_Victory& Prototype)
	: CUIObj { Prototype }
{
}
...
HRESULT CUI_Victory::Initialize(void* pArg)
{
	...
	Desc.fSizeY = 136.f;
	lstrcpy(Desc.szName, TEXT("Victory"));
...
void CUI_Victory::Update(_float fTimeDelta)
{
...
void CUI_Victory::Update_Late(_float fTimeDelta)
{
	if (m_bVisible == false)
		return;
...
HRESULT CUI_Victory::Ready_Components()
{
	...
	// Com_Texture 추가
	if (FAILED(__super::Add_Component(ENUM_TO_UINT(LEVELID::GAMEPLAY), TEXT("Prototype_Component_Texture_UI_Victory"),
...
HRESULT CUI_Victory::Bind_ShaderResources()
{
...
CGameObject* CUI_Victory::Clone(void* pArg)
{
	...
	{
		MSG_BOX("Failed to Clone : CVictory");
		Safe_Release(pInstance);
	}
