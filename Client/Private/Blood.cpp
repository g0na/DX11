...
{   
    lstrcpy(m_szName, TEXT("Effect_Blood"));
...
void CBlood::Update_Priority(_float fTimeDelta)
{
...
void CBlood::Update_Late(_float fTimeDelta)
{
    if (!m_bIsOn)
        return;
...
void CBlood::Play(_fvector vResetPosition, _fvector vTargetPosition)
{
    m_bIsOn = true;
...
{
    /* For.Com_VIBuffer */
    if (FAILED(__super::Add_Component(ENUM_TO_UINT(LEVELID::GAMEPLAY), TEXT("Prototype_Component_VIBuffer_Particle_Blood"),
