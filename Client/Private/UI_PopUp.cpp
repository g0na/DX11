...
HRESULT CUI_PopUp::Initialize(void* pArg)
{
	...
	Desc.fSizeX = 850.f;
	Desc.fSizeY = 70.f;
	lstrcpy(Desc.szName, TEXT("PopUp"));
...
void CUI_PopUp::Update_Priority(_float fTimeDelta)
{
}
...
void CUI_PopUp::Update_Late(_float fTimeDelta)
{
	if (m_bVisible == false)
		return;
...
HRESULT CUI_PopUp::Render()
{
	...
	m_pGameInstance->Draw_Text(TEXT("Font_Korean"), TEXT("E : 사다리를 오른다."), _float2((_float)(g_iWinSizeX * 0.414f), g_iWinSizeY * 0.8625f), XMVectorSet(1.f, 1.f, 1.f, 1.f));

	return S_OK;
}

HRESULT CUI_PopUp::Ready_Components()
{
	// Com_VIBuffer 추가
...
HRESULT CUI_PopUp::Bind_ShaderResources()
{
