...
void CPlayer_Idle::Update_State(_float fTimeDelta)
{
    ...
    // 문 열기
    if (m_pStateMachine->Get_BoolData(TEXT("Player_Door"), false) == true)
    {
        m_pStateMachine->Change_State(CPlayer::DOOR);
        return;
    }

    // 아이템 획득
    if (m_pStateMachine->Get_BoolData(TEXT("Player_PickUp"), false) == true)
    {
        m_pStateMachine->Change_State(CPlayer::PICKUP);
        return;
    }
