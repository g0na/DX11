...
void CPlayer_Walk::Update_State(_float fTimeDelta)
{
    ...
    // 사운드
    if (m_pPlayerModel->Get_Animation(4)->Get_CurrentTrackPosition() >= 0.166f &&
        m_pPlayerModel->Get_Animation(4)->Get_CurrentTrackPosition() <= 0.186f)
        m_pGameInstance->PlaySoundW(TEXT("Player_Walk1.wav"), CHANNELID::SOUND_EFFECT, 1.f);
    else if (m_pPlayerModel->Get_Animation(4)->Get_CurrentTrackPosition() >= 0.566f &&
        m_pPlayerModel->Get_Animation(4)->Get_CurrentTrackPosition() <= 0.586f)
